...
class PXL_Taxonomy_Meta
{
    ...
    function __construct()
    {
        ...
        do_action('pxl_taxonomy_meta_register', $this);
    ...
    function admin_init()
    {
        ...
        if ('term.php' == $pagenow) {
            $this->notices = get_transient('pxl-taxonomy-meta-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('pxl-taxonomy-meta-transients', '');
            } else {
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_taxonomy_metabox' : 'pxl_taxonomy_metabox';
    ...
    { 
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$taxonomy])) {
                trigger_error(sprintf(esc_html__('The field with id %1$s for taxonomy %2$s is already registered.', PXL_TEXT_DOMAIN), esc_html($field['id'], ETCTEXT_DOMAIN), esc_html($taxonomy, ETCTEXT_DOMAIN)));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $taxonomy)) {
                trigger_error(sprintf(esc_html__('Section %1$s for taxonomy %2$s is already exist.', PXL_TEXT_DOMAIN), esc_html($field['id']), esc_html($taxonomy)));
                return;
            } else {
    ...
    {
        wp_nonce_field('pxl_taxonomy_metabox_nonce_action', 'pxl_taxonomy_metabox_nonce');
    ...
    function panel_scripts()
    {
        ...
        wp_enqueue_script('pxl-metabox', PXL_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js'), self::$version, 'all');
    ...
    protected function save_term_meta($term_id, $tt_id, $taxonomy, $transient = false)
    {
        ...
        if (empty($_POST[$opt_name]) || empty($_POST['pxl_taxonomy_metabox_nonce']) || !wp_verify_nonce($_POST['pxl_taxonomy_metabox_nonce'], 'pxl_taxonomy_metabox_nonce_action')) {
            return;
        }
        ...
        if ($transient) {
            ...
            if (!empty($redux->errors) || !empty($redux->warnings)) {
                ...
                set_transient('pxl-taxonomy-metabox-transients', $notices);
            }
        }
    }
...
//        }
    ...
    protected function generate_output_css($taxonomy, $options)
    {
        ...
        echo '<style id="pxl-' . $taxonomy . '-dynamic-css" data-type="redux-output-css">' . $redux->outputCSS . '</style>';
    }
}
