...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$yhsshu_import_demo_id = get_option('yhsshu_import_demo_id',false);
		if( !empty($yhsshu_import_demo_id) ){
			return;
		}
	...
	public function admin_page() {
		...
		$theme_slug = apply_filters( 'yhsshu_demo_item_download', get_option( 'template' ) );
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'yhsshu' ),
		);
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'yhsshu' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'yhsshu' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'yhsshu' ),
		...
		$this->steps['content'] = array(
			'name' => esc_html__( 'Content', 'yhsshu' ),
			'view' => array( $this, 'content' ),
		);

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'yhsshu' ),
		...
		<div class="merlin__content--transition">
			...
			<?php  if ( ! $is_theme_registered ) : ?>
				<div class="merlin__content--license-key">
					<div class="yhsshu-regis-box yhsshu-regis-box-solid yhsshu-regis-register-box">
						<div class="yhsshu-regis-box-head">
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'yhsshu' ); ?>">
										<?php esc_html_e( 'Required', 'yhsshu' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'yhsshu' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'yhsshu' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( sanitize_title($_POST['slug']) === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'yhsshu' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'yhsshu' );
			wp_send_json( $json );
		} else {
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'yhsshu' ),
				)
			);
		}
